 ...
 *
 * Subscribes to Supabase real-time updates for the posts table.
...
export interface UseRealtimePostsResult {
  /**
 ...
 * ```tsx
 * function BlogPage() {
 *   const { status, lastUpdate } = useRealtimePosts();
 *
 *   return (
 *     <div>
...
export function useRealtimePosts(
  ...
  const handleUpdate = useCallback(
    throttle(() => {
      ...
      fetch('/api/revalidate?tag=posts', { method: 'POST' }).catch(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: RealtimePostgresChangesPayload<Record<string, unknown>>) => {
          console.log('[Realtime] Posts update:', payload.eventType, payload);
